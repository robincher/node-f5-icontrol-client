...
let IControlUtil = function(opts) {
    ...
    if (_.isNil(opts.token)) {
        return Promise.reject(new Error('iControl.sendRequest(): Authorization Token is required'));
    }

    if(strict && _.isNil(opts.ca)) {
        return Promise.reject(new Error('iControl.sendRequest(): No CA Specify for strict mode'));
    }
